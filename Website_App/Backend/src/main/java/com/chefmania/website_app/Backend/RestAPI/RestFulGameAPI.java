...
//// api/game/cards/player    → get player's cards
//// api/game/cards/computer  – get computer's cards
...
public class RestFulGameAPI {
    ...
    public List<Coordinates> getValidMoves(@RequestBody ValidMoveRequest request) {
    ...
    public GameController makePlayerMove(@RequestBody ChefManiaMoveClass chefMania) {
        Player current = chefMania.isChef()
            ? service.getGame().getComputer()
            : service.getGame().getPlayer();

        service.makeMove(
            chefMania.getFrom(),
            chefMania.getTo(),
            chefMania.getCardUsed(),
            current
        );
